...
const EVENT_CONFIG: Record<ActivityType, {
  ...
  complete: {
    icon: "✓", color: "var(--green)", bg: "var(--green-soft)",
    text: (e) => `completed ${e.planTitle}`,
  },
  fail: {
    icon: "✗", color: "var(--red)", bg: "var(--red-soft)",
    text: (e) => `missed ${e.planTitle}`,
  },
  join: {
    icon: "👋", color: "var(--brand)", bg: "var(--brand-soft)",
    text: (e) => e.meta?.participantCount
      ? `joined ${e.planTitle} · ${e.meta.participantCount} people in`
      : `joined ${e.planTitle}`,
  },
  create: {
    icon: "+", color: "#0891B2", bg: "#DBEAFE",
    text: (e) => `created ${e.planTitle}`,
  },
  award: {
    icon: "⬡", color: "#D97706", bg: "#FEF3C7",
    text: (e) => e.meta?.creditDelta
...
function EventItem({ event, isNew }: { event: ActivityEvent; isNew?: boolean }) {
  const cfg = EVENT_CONFIG[event.type];
  const isMe = event.userId === "me";
  const name = isMe ? "You" : event.userInitials;

  return (
    <div className={`${styles.eventItem} ${isNew ? styles.newEvent : ""}`}>
      {/* Avatar */}
      <div className={styles.avatar} style={{ background: event.userColor }}>
        {event.userInitials}
        <div className={styles.eventBadge} style={{ background: cfg.bg, color: cfg.color }}>
          {cfg.icon}
        </div>
      </div>
      ...
      <div className={styles.content}>
        <div className={styles.eventText}>
          <strong>{name}</strong>{" "}
          <span>{cfg.text(event)}</span>
        </div>

        {event.meta?.creditDelta !== undefined && (
          <div className={styles.creditPill} 
               style={{
                 background: event.meta.creditDelta > 0 ? "var(--green-soft)" : "var(--red-soft)",
                 color: event.meta.creditDelta > 0 ? "var(--green)" : "var(--red)",
               }}>
            {event.meta.creditDelta > 0 ? "+" : ""}{event.meta.creditDelta} credits
          </div>
        )}
      </div>
      ...
      {events.length === 0 ? (
        <div className={styles.emptyStateContainer}>
          <EmptyState
            ...
            action={{ label: "Create a plan", href: "/create" }}
          />
        </div>
      ) : (
